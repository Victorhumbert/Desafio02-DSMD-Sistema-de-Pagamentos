...
async function connect(retries = 0) {
  while (true) {
    try {
      ...
      conn.on('error', () => {
        console.error('[publisher] connection error');
        channel = null;
      });
      conn.on('close', () => {
        console.error('[publisher] connection closed, will retry');
        channel = null;
      ...
      console.error(`[publisher] connect failed (${err.message || err}). retrying in ${wait}ms`);
...
async function send(routingKey, payload) {
